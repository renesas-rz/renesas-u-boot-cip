...
static ulong rzv2l_cpg_clk_get_rate_by_id(struct udevice *dev, unsigned int id);
static ulong rzv2l_cpg_clk_get_rate_by_name(struct udevice *dev, const char *name);

struct rzv2l_cpg_data {
	void __iomem *base;
	struct rzv2l_cpg_info *info;
};
...
static int rzv2l_cpg_clk_set(struct clk *clk, bool enable)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(clk->dev);
	const unsigned int cpg_clk_id = CPG_CLK_ID(clk->id);
	const struct rzv2l_mod_clk *mod_clk = NULL;
	u32 value;
	unsigned int i;
...
static int rzv2l_cpg_clk_enable(struct clk *clk)
{
	return rzv2l_cpg_clk_set(clk, true);
}

static int rzv2l_cpg_clk_disable(struct clk *clk)
{
	return rzv2l_cpg_clk_set(clk, false);
}

static int rzv2l_cpg_clk_of_xlate(struct clk *clk,
...
static ulong rzv2l_sdhi_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(dev);
	...
	return rzv2l_cpg_clk_get_rate_by_name(dev, cc->parent_names[sel - 1]);
}

static ulong rzv2l_div_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(dev);
	const ulong offset = CPG_CONF_OFFSET(cc->conf);
	...
	for (i = 0; cc->dtable[i].div; i++) {
		if (cc->dtable[i].val == sel)
			return rzv2l_cpg_clk_get_rate_by_id(dev, cc->parent) / cc->dtable[i].div;
	}
	dev_err(dev, "Invalid selector value %u for clock %s\n", sel, cc->name);
	return -EINVAL;
}

static ulong rzv2l_core_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	ulong parent_rate;
	struct clk clk_in;
	switch (cc->type) {
	case CLK_TYPE_FF:
		parent_rate = rzv2l_cpg_clk_get_rate_by_id(dev, cc->parent);
		return parent_rate * cc->mult / cc->div;
	case CLK_TYPE_IN:
		clk_get_by_name(dev, cc->name, &clk_in);
		return clk_get_rate(&clk_in);
	case CLK_TYPE_SD_MUX:
		return rzv2l_sdhi_clk_get_rate(dev, cc);
	case CLK_TYPE_DIV:
		return rzv2l_div_clk_get_rate(dev, cc);
	default:
...
static ulong rzv2l_cpg_clk_get_rate_by_id(struct udevice *dev, unsigned int id)
{
	...
	if (is_mod_clk(id)) {
		for (i = 0; i < data->info->num_mod_clks; i++) {
			if (data->info->mod_clks[i].id == cpg_clk_id)
				return rzv2l_cpg_clk_get_rate_by_id(dev,
	...
	for (i = 0; i < data->info->num_core_clks; i++) {
		if (data->info->core_clks[i].id == cpg_clk_id)
			return rzv2l_core_clk_get_rate(dev, &data->info->core_clks[i]);
	}
...
static ulong rzv2l_cpg_clk_get_rate_by_name(struct udevice *dev, const char *name)
{
	...
	for (i = 0; i < data->info->num_mod_clks; i++) {
		if (!strcmp(name, data->info->mod_clks[i].name))
			return rzv2l_cpg_clk_get_rate_by_id(dev, data->info->mod_clks[i].parent);
	}
	for (i = 0; i < data->info->num_core_clks; i++) {
...
static ulong rzv2l_cpg_clk_get_rate(struct clk *clk)
{
	return rzv2l_cpg_clk_get_rate_by_id(clk->dev, clk->id);
}

static ulong rzv2l_sdhi_clk_set_rate(struct udevice *dev, const struct cpg_core_clk *cc, ulong rate)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(dev);
	const ulong offset = CPG_CONF_OFFSET(cc->conf);
	...
	for (i = 0; i < cc->num_parents; i++) {
		target_rate = rzv2l_cpg_clk_get_rate_by_name(dev, cc->parent_names[i]);
		if (rate >= target_rate) {
			new_sel = i + 1;
			break;
		}
...
static ulong rzv2l_core_clk_set_rate(struct udevice *dev, const struct cpg_core_clk *cc, ulong rate)
{
	if (cc->type == CLK_TYPE_SD_MUX)
		return rzv2l_sdhi_clk_set_rate(dev, cc, rate);
	...
	if (cc->id == CLK_SD0_DIV4 || cc->id == CLK_SD1_DIV4)
		return rzv2l_core_clk_get_rate(dev, cc);

	dev_err(dev, "set_rate needed for clock %u, type %d\n", cc->id, cc->type);
	return -ENOSYS;
}

static ulong rzv2l_cpg_clk_set_rate_by_id(struct udevice *dev, unsigned int id, ulong rate)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(dev);
	const unsigned int cpg_clk_id = CPG_CLK_ID(id);
	...
	if (is_mod_clk(id)) {
		for (i = 0; i < data->info->num_mod_clks; i++) {
			if (data->info->mod_clks[i].id == cpg_clk_id)
				return rzv2l_cpg_clk_set_rate_by_id(dev,
								    data->info->mod_clks[i].parent,
								    rate);
		}
	...
	for (i = 0; i < data->info->num_core_clks; i++) {
		if (data->info->core_clks[i].id == cpg_clk_id)
			return rzv2l_core_clk_set_rate(dev, &data->info->core_clks[i], rate);
	}
...
static ulong rzv2l_cpg_clk_set_rate(struct clk *clk, ulong rate)
{
	return rzv2l_cpg_clk_set_rate_by_id(clk->dev, clk->id, rate);
}

static const struct clk_ops rzv2l_cpg_clk_ops = {
	.enable		= rzv2l_cpg_clk_enable,
	.disable	= rzv2l_cpg_clk_disable,
	.of_xlate	= rzv2l_cpg_clk_of_xlate,
	.get_rate	= rzv2l_cpg_clk_get_rate,
	.set_rate	= rzv2l_cpg_clk_set_rate,
};

U_BOOT_DRIVER(rzv2l_cpg_clk) = {
	.name		= "rzv2l-cpg-clk",
	.id		= UCLASS_CLK,
	.ops		= &rzv2l_cpg_clk_ops,
	.flags		= DM_FLAG_VITAL,
};

static int rzv2l_cpg_rst_set(struct reset_ctl *reset_ctl, bool asserted)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(reset_ctl->dev);
	const struct rzv2l_reset *rst;
	u32 value;
...
static int rzv2l_cpg_rst_assert(struct reset_ctl *reset_ctl)
{
	return rzv2l_cpg_rst_set(reset_ctl, true);
}

static int rzv2l_cpg_rst_deassert(struct reset_ctl *reset_ctl)
{
	return rzv2l_cpg_rst_set(reset_ctl, false);
}

static int rzv2l_cpg_rst_of_xlate(struct reset_ctl *reset_ctl,
				  struct ofnode_phandle_args *args)
{
	struct rzv2l_cpg_data *data =
		(struct rzv2l_cpg_data *)dev_get_driver_data(reset_ctl->dev);
...
static const struct reset_ops rzv2l_cpg_rst_ops = {
	.rst_assert	= rzv2l_cpg_rst_assert,
	.rst_deassert	= rzv2l_cpg_rst_deassert,
	.of_xlate	= rzv2l_cpg_rst_of_xlate,
};

U_BOOT_DRIVER(rzv2l_cpg_rst) = {
	.name		= "rzv2l-cpg-rst",
	.id		= UCLASS_RESET,
	.ops		= &rzv2l_cpg_rst_ops,
	.flags		= DM_FLAG_VITAL,
};

int rzv2l_cpg_bind(struct udevice *parent)
{
	...
	data->info = (struct rzv2l_cpg_info *)dev_get_driver_data(parent);
	if (!data->info)
		return -EINVAL;

	drv = lists_driver_lookup_name("rzv2l-cpg-clk");
	if (!drv)
		return -ENOENT;
	...
	drv = lists_driver_lookup_name("rzv2l-cpg-rst");
	if (!drv) {
		device_unbind(cdev);
		return -ENOENT;
	}
