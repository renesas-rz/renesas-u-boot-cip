...
static ulong rzg2ul_cpg_clk_get_rate_by_id(struct udevice *dev, unsigned int id);
static ulong rzg2ul_cpg_clk_get_rate_by_name(struct udevice *dev, const char *name);

struct rzg2ul_cpg_data {
	void __iomem *base;
	struct rzg2ul_cpg_info *info;
};
...
static int rzg2ul_cpg_clk_set(struct clk *clk, bool enable)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(clk->dev);
	const unsigned int cpg_clk_id = CPG_CLK_ID(clk->id);
	const struct rzg2ul_mod_clk *mod_clk = NULL;
	u32 value;
	unsigned int i;
...
static int rzg2ul_cpg_clk_enable(struct clk *clk)
{
	return rzg2ul_cpg_clk_set(clk, true);
}

static int rzg2ul_cpg_clk_disable(struct clk *clk)
{
	return rzg2ul_cpg_clk_set(clk, false);
}

static int rzg2ul_cpg_clk_of_xlate(struct clk *clk,
...
static ulong rzg2ul_sdhi_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(dev);
	...
	return rzg2ul_cpg_clk_get_rate_by_name(dev, cc->parent_names[sel - 1]);
}

static ulong rzg2ul_div_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(dev);
	const ulong offset = CPG_CONF_OFFSET(cc->conf);
	...
	for (i = 0; cc->dtable[i].div; i++) {
		if (cc->dtable[i].val == sel)
			return rzg2ul_cpg_clk_get_rate_by_id(dev, cc->parent) / cc->dtable[i].div;
	}
	dev_err(dev, "Invalid selector value %u for clock %s\n", sel, cc->name);
	return -EINVAL;
}

static ulong rzg2ul_core_clk_get_rate(struct udevice *dev, const struct cpg_core_clk *cc)
{
	ulong parent_rate;
	struct clk clk_in;
	switch (cc->type) {
	case CLK_TYPE_FF:
		parent_rate = rzg2ul_cpg_clk_get_rate_by_id(dev, cc->parent);
		return parent_rate * cc->mult / cc->div;
	case CLK_TYPE_IN:
		clk_get_by_name(dev, cc->name, &clk_in);
		return clk_get_rate(&clk_in);
	case CLK_TYPE_SD_MUX:
		return rzg2ul_sdhi_clk_get_rate(dev, cc);
	case CLK_TYPE_DIV:
		return rzg2ul_div_clk_get_rate(dev, cc);
	default:
...
static ulong rzg2ul_cpg_clk_get_rate_by_id(struct udevice *dev, unsigned int id)
{
	...
	if (is_mod_clk(id)) {
		for (i = 0; i < data->info->num_mod_clks; i++) {
			if (data->info->mod_clks[i].id == cpg_clk_id)
				return rzg2ul_cpg_clk_get_rate_by_id(dev,
	...
	for (i = 0; i < data->info->num_core_clks; i++) {
		if (data->info->core_clks[i].id == cpg_clk_id)
			return rzg2ul_core_clk_get_rate(dev, &data->info->core_clks[i]);
	}
...
static ulong rzg2ul_cpg_clk_get_rate_by_name(struct udevice *dev, const char *name)
{
	...
	for (i = 0; i < data->info->num_mod_clks; i++) {
		if (!strcmp(name, data->info->mod_clks[i].name))
			return rzg2ul_cpg_clk_get_rate_by_id(dev, data->info->mod_clks[i].parent);
	}
	for (i = 0; i < data->info->num_core_clks; i++) {
...
static ulong rzg2ul_cpg_clk_get_rate(struct clk *clk)
{
	return rzg2ul_cpg_clk_get_rate_by_id(clk->dev, clk->id);
}

static ulong rzg2ul_sdhi_clk_set_rate(struct udevice *dev, const struct cpg_core_clk *cc, ulong rate)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(dev);
	const ulong offset = CPG_CONF_OFFSET(cc->conf);
	...
	for (i = 0; i < cc->num_parents; i++) {
		target_rate = rzg2ul_cpg_clk_get_rate_by_name(dev, cc->parent_names[i]);
		if (rate >= target_rate) {
			new_sel = i + 1;
			break;
		}
...
static ulong rzg2ul_core_clk_set_rate(struct udevice *dev, const struct cpg_core_clk *cc, ulong rate)
{
	if (cc->type == CLK_TYPE_SD_MUX)
		return rzg2ul_sdhi_clk_set_rate(dev, cc, rate);
	...
	if (cc->id == CLK_SD0_DIV4 || cc->id == CLK_SD1_DIV4)
		return rzg2ul_core_clk_get_rate(dev, cc);

	dev_err(dev, "set_rate needed for clock %u, type %d\n", cc->id, cc->type);
	return -ENOSYS;
}

static ulong rzg2ul_cpg_clk_set_rate_by_id(struct udevice *dev, unsigned int id, ulong rate)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(dev);
	const unsigned int cpg_clk_id = CPG_CLK_ID(id);
	...
	if (is_mod_clk(id)) {
		for (i = 0; i < data->info->num_mod_clks; i++) {
			if (data->info->mod_clks[i].id == cpg_clk_id)
				return rzg2ul_cpg_clk_set_rate_by_id(dev,
								    data->info->mod_clks[i].parent,
								    rate);
		}
	...
	for (i = 0; i < data->info->num_core_clks; i++) {
		if (data->info->core_clks[i].id == cpg_clk_id)
			return rzg2ul_core_clk_set_rate(dev, &data->info->core_clks[i], rate);
	}
...
static ulong rzg2ul_cpg_clk_set_rate(struct clk *clk, ulong rate)
{
	return rzg2ul_cpg_clk_set_rate_by_id(clk->dev, clk->id, rate);
}

static const struct clk_ops rzg2ul_cpg_clk_ops = {
	.enable		= rzg2ul_cpg_clk_enable,
	.disable	= rzg2ul_cpg_clk_disable,
	.of_xlate	= rzg2ul_cpg_clk_of_xlate,
	.get_rate	= rzg2ul_cpg_clk_get_rate,
	.set_rate	= rzg2ul_cpg_clk_set_rate,
};

U_BOOT_DRIVER(rzg2ul_cpg_clk) = {
	.name		= "rzg2ul-cpg-clk",
	.id		= UCLASS_CLK,
	.ops		= &rzg2ul_cpg_clk_ops,
	.flags		= DM_FLAG_VITAL,
};

static int rzg2ul_cpg_rst_set(struct reset_ctl *reset_ctl, bool asserted)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(reset_ctl->dev);
	const struct rzg2ul_reset *rst;
	u32 value;
...
static int rzg2ul_cpg_rst_assert(struct reset_ctl *reset_ctl)
{
	return rzg2ul_cpg_rst_set(reset_ctl, true);
}

static int rzg2ul_cpg_rst_deassert(struct reset_ctl *reset_ctl)
{
	return rzg2ul_cpg_rst_set(reset_ctl, false);
}

static int rzg2ul_cpg_rst_of_xlate(struct reset_ctl *reset_ctl,
				  struct ofnode_phandle_args *args)
{
	struct rzg2ul_cpg_data *data =
		(struct rzg2ul_cpg_data *)dev_get_driver_data(reset_ctl->dev);
...
static const struct reset_ops rzg2ul_cpg_rst_ops = {
	.rst_assert	= rzg2ul_cpg_rst_assert,
	.rst_deassert	= rzg2ul_cpg_rst_deassert,
	.of_xlate	= rzg2ul_cpg_rst_of_xlate,
};

U_BOOT_DRIVER(rzg2ul_cpg_rst) = {
	.name		= "rzg2ul-cpg-rst",
	.id		= UCLASS_RESET,
	.ops		= &rzg2ul_cpg_rst_ops,
	.flags		= DM_FLAG_VITAL,
};

int rzg2ul_cpg_bind(struct udevice *parent)
{
	...
	data->info = (struct rzg2ul_cpg_info *)dev_get_driver_data(parent);
	if (!data->info)
		return -EINVAL;

	drv = lists_driver_lookup_name("rzg2ul-cpg-clk");
	if (!drv)
		return -ENOENT;
	...
	drv = lists_driver_lookup_name("rzg2ul-cpg-rst");
	if (!drv) {
		device_unbind(cdev);
		return -ENOENT;
	}
