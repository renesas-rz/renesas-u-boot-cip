...
static int riic_probe(struct udevice *dev)
{
    ...
    if (ret) {
        printf("RIIC: ERROR clk_get_by_index failed\n");
        return ret;
    }

    ret = clk_enable(&priv->clk);
    if (ret) {
        printf("RIIC: ERROR clk_enable failed\n");
        return ret;
    }
...
static const struct udevice_id riic_ids[] = {
	{ .compatible = "renesas,riic-r9a07g076", },
	{ }
};

U_BOOT_DRIVER(riic_rzt2n) = {
	.name           = "riic_rzt2n",
	.id             = UCLASS_I2C,
	.of_match       = riic_ids,
	.probe          = riic_probe,
