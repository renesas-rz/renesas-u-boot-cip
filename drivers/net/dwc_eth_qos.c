...
static ulong eqos_get_tick_clk_rate_renesas_rz(struct udevice *dev)
{
       return 125000000;
}
...
static int eqos_set_tx_clk_speed_renesas_rz(struct udevice *dev)
{
...
static int eqos_start(struct udevice *dev)
{
	...
	for (i = 0; i < EQOS_DESCRIPTORS_RX; i++) {
		...
		rx_desc->des0 = (u32)(ulong)((u64)eqos->rx_dma_buf + (i * EQOS_MAX_PACKET_SIZE));
	...
	writel((ulong)((u64)eqos_get_desc(eqos, 0, false)), &eqos->dma_regs->ch0_txdesc_list_address);
	writel(EQOS_DESCRIPTORS_TX - 1,
	       &eqos->dma_regs->ch0_txdesc_ring_length);

	writel(0, &eqos->dma_regs->ch0_rxdesc_list_haddress);
	writel((ulong)((u64)eqos_get_desc(eqos, 0, true)), &eqos->dma_regs->ch0_rxdesc_list_address);
	 ...
	 * available.
	 */
	last_rx_desc = (ulong)((u64)eqos_get_desc(eqos, EQOS_DESCRIPTORS_RX - 1, true));
...
static int eqos_send(struct udevice *dev, void *packet, int length)
{
	...
	tx_desc->des0 = (ulong)((u64)eqos->tx_dma_buf);
	tx_desc->des1 = 0;
	tx_desc->des2 = length;
	/*
	...
	writel((ulong)((u64)eqos_get_desc(eqos, eqos->tx_desc_idx, false)), &eqos->dma_regs->ch0_txdesc_tail_pointer);
...
static int eqos_free_pkt(struct udevice *dev, uchar *packet, int length)
{
	...
	eqos->config->ops->eqos_flush_desc(rx_desc);
	eqos->config->ops->eqos_inval_buffer(packet, length);
	rx_desc->des0 = (u32)(ulong)((u64)packet);
	rx_desc->des1 = 0;
	rx_desc->des2 = 0;
	/*
	 * Make sure that if HW sees the _OWN write below, it will see all the
	...
	rx_desc->des3 = EQOS_DESC3_OWN | EQOS_DESC3_BUF1V;
	eqos->config->ops->eqos_flush_desc(rx_desc);
	writel((ulong)((u64)rx_desc), &eqos->dma_regs->ch0_rxdesc_tail_pointer);
...
static int eqos_probe_resources_renesas_rz(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static phy_interface_t eqos_get_interface_renesas_rz(struct udevice *dev)
{
...
static int eqos_remove_resources_renesas_rz(struct udevice *dev)
{
       /* Board init */
...
static struct eqos_ops eqos_renesas_rz_ops = {
       .eqos_inval_desc = eqos_inval_desc_generic,
       .eqos_flush_desc = eqos_flush_desc_generic,
       .eqos_inval_buffer = eqos_inval_buffer_generic,
       .eqos_flush_buffer = eqos_flush_buffer_generic,
       .eqos_probe_resources = eqos_probe_resources_renesas_rz,
       .eqos_remove_resources = eqos_remove_resources_renesas_rz,
       ...
       .eqos_set_tx_clk_speed = eqos_set_tx_clk_speed_renesas_rz,
       .eqos_get_tick_clk_rate = eqos_get_tick_clk_rate_renesas_rz
};

static const struct eqos_config __maybe_unused eqos_renesas_rz_config = {
       ...
       .axi_bus_width = EQOS_AXI_WIDTH_128,
       .interface = eqos_get_interface_renesas_rz,
       .ops = &eqos_renesas_rz_ops
};
...
#if IS_ENABLED(CONFIG_DWC_ETH_QOS_RZT2N)
	{
		.compatible = "renesas,rzt2n-eqos",
		.data = (ulong)&eqos_renesas_rz_config
	},
#endif
	{ }
};
