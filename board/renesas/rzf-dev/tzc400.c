...
#define BIT_32(nr)			((unsigned)(1) << (nr))
				...
				int val)
{
	...
	printf("%s open_status:%u val:%d filter:%u\n", __func__, open_status, val, filter);
...
// 		unsigned int cur_status = get_gate_keeper_os(base);
// 		if (cur_status != open_status)
// 			printf("%s prev:%u cur:%u raw:%u\n", __func__, open_status, cur_status, get_gate_keeper_os(base));
// 		else
// 			break;
	}
}
...
void tzc400_enable_filters(void)
{
	...
	for (filter = 0U; filter < tzc400.num_filters; filter++) {
		...
		if (state != 0U) {
			 ...
			 */
			puts("TZC-400 : Filter Gatekeeper already enabled.\n");
// 			panic();
		}
